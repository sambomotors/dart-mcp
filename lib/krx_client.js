...
export async function queryKrx({ service, stock_code, bas_dd }, {
  ...
  if (row.BAS_DD == null || String(row.BAS_DD).trim() === "") {
    return result("DATE_UNAVAILABLE", dateDiagnostics);
  }
  if (typeof row.BAS_DD !== "string" || !validDate(row.BAS_DD)) {
    return result("SCHEMA_ERROR", dateDiagnostics);
  }
  if (row.BAS_DD !== bas_dd) return result("DATE_MISMATCH", dateDiagnostics);
  ...
  return result("OK", { basis_date: row.BAS_DD, data: numbers, raw_fields: raw,
